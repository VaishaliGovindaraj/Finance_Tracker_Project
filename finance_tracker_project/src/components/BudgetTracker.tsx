...
export default function BudgetTracker({ budgets }: BudgetTrackerProps) {
  ...
  return (
    <div className="bg-white p-4 sm:p-6 rounded-lg shadow-md">
      <h2 className="text-xl sm:text-2xl font-bold mb-4 text-gray-800">Budget Overview</h2>
      ...
      {budgets.length === 0 ? (
        ...
        <div className="space-y-3 sm:space-y-4">
          {budgets.map((budget) => {
            ...
            return (
              <div key={budget.id} className="border border-gray-200 rounded-lg p-3 sm:p-4">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center mb-2 gap-1">
                  <h3 className="font-semibold text-gray-800 text-sm sm:text-base">{budget.category}</h3>
